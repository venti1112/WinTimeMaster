...
LockController::LockController(TimeRuleModel *model, QQmlApplicationEngine *engine, QObject *parent) : QObject(parent), m_model(model), m_engine(engine) {
    if (m_engine) {
        ...
        if (component.isError()) qWarning() << "Failed to load LockScreen.qml:" << component.errorString();
            ...
            if (m_lockWindow) m_lockWindow->setVisible(false);
            else {
...
void LockController::applySystemRestrictions() {
    ...
    if (cfg->readBool("EnableInputBlock", true)) blockInput(true);
    if (cfg->readBool("DisableTaskManager", true)) disableTaskManager(true);
    if (cfg->readBool("KillTaskmgr", true)) QProcess::execute("taskkill", QStringList() << "/f" << "/im" << "taskmgr.exe");
}

void LockController::removeSystemRestrictions() {
    ConfigManager *cfg = ConfigManager::instance();
    if (cfg->readBool("EnableInputBlock", true)) blockInput(false);
    if (cfg->readBool("DisableTaskManager", true)) disableTaskManager(false);
}
